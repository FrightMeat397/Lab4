	...
	sealed class Interpreter : IStatementVisitor, IExpressionVisitor<object> {
		...
		public void VisitWhile(While whileStatement) {
			...
			while (Calc<bool>(whileStatement.Condition)) {
				if (!RunBlock(whileStatement.Body)) {
					breakNestedLevel--;
					break;
				}
			}
		...
		public void VisitBreak(Break breakStatement) {
			if (0 < breakStatement.LoopCount && breakStatement.LoopCount <= currentLevel) {
				breakNestedLevel = breakStatement.LoopCount;
			}
			else {
				throw new Exception($"Число {breakStatement.LoopCount} при break больше уровня вложенности {currentLevel} или равно 0");
			}
		}
